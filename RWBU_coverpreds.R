#Ensemble predictions for Rwanda, Burundi, Kivu, Central Africa Highlands
...
dir.create("CA_data", showWarnings=F)
dat_dir <- "CA_data"

#download Geosurvey data  28000 + for Woodland
download.file("https://www.dropbox.com/s/vp4fe085p2c7u01/central-african-highl_hsp.zip?raw=1", "./CA_data/central-african-highl_hsp.zip", mode="wb")
unzip("./central-african-highl_hsp.zip", exdir= "./CA_data", overwrite=T)
geos1 <- read.table(paste(dat_dir, "/central-african-highl_hsp.csv", sep=""), header=T, sep=",")
#geos1 <- na.omit(geos1)
# Project GeoSurvey coords to grid CRS for 28000+
...
# download CA Hghlands  Gtifs (!!!~552 MB) and stack in raster
#download.file("https://www.dropbox.com/s/dzfiuvl35430nl8/CA_HGHLDS.zip?raw=1", "./CA_data/CA_HGHLDS.zip", mode="wb")
#unzip("./CA_data/CA_HGHLDS.zip", exdir="./CA_data", overwrite=T)
glist <- list.files(path="./CA_data", pattern="tif", full.names=T)
grid <- stack(glist)
t=scale(grid,center=TRUE, scale=TRUE)
#names(t)=c("Al", "BIO12ALT", "BIOMFI", "BNALT", "BNAsd", "BNAvar", "BNASavg","BNASsd", "BNASv",
#           "CEC", "PREC_2014", "ECN", "Ca", "K", "Mg", "Na", "LSTD", "LSTND",
#           "EVI", "NDVI", "B1", "B2", "B3", "B7", "NTOT", "ORC", "PH", "RIVDIST", "RDSDIST",
#           "SENT1VV", "SENT1VH", "SND", "FPARavg", "FPARsd", "FPARvar", "ELEV", 
#           "TRMMavg", "TWI")
#+ Data setup --------------------------------------------------------------
# Extract gridded variables at GeoSurvey locations
geogrid=extract(t, geos1.proj) #extract 28k
...
plot(varImp(WDL1.rf, scale=F), main= "Variable Importance Random Forest Woodland", cex=1.2, col= "black", top=30)
...
plot(WDL1.imp, main= "Variable Importance Neural Net Woodland", cex=1.2, col= "black", top=30)
WDL1nn.pred <- predict(t, WDL1.nn, type="prob")

#k nearest neighbor classification
WDL1.kn <- train(WDL1 ~., data=wdl1Train, 
                 method = "knn",
                 metric= "ROC",
                 #preProc = c("center", "scale"), 
                 trControl = objControl)
confusionMatrix(WDL1.kn)
WDL1.imp <- varImp(WDL1.kn, useModel = FALSE)
plot(WDL1.imp, main= "Variable Importance k nearest neighbor Woodland", cex=1.2, col= "black", top=30)
WDL1kn.pred <- predict(t, WDL1.kn, type="prob")

#ensemble regression glmnet (elastic net)
pred <- stack(1-WDL1rf.pred, 
              1-WDL1gbm.pred, 1-WDL1nn.pred, 1-WDL1kn.pred)
names(pred) <- c("WDL1rf","WDL1gbm", "WDL1nn", "WDL1kn")
plot(pred)
...
#confusion matrix
predictions= predict(WDL1.ens, WDL1ensTest)
confusionMatrix(predictions, WDL1ensTest [,1])
...
#k nearest neighbor
WDL.kn <- train(WDL1 ~., data=wdl1dat, 
                method = "knn", 
                metric= "ROC",
                #preProc = c("center", "scale"), 
                trControl = objControl)
confusionMatrix((WDL.kn))
WDLkn.pred <- predict(t, WDL.kn, type="prob")
#ensemble regression glmnet (elastic net)
pred <- stack(1-WDLrf.pred, 
              1-WDLgbm.pred, 1-WDLnn.pred, 1-WDLkn.pred)
names(pred) <- c("WDLrf","WDLgbm", "WDLnn", "WDLkn")
geospred <- extract(pred, geos1.proj)
...
#create results
dir.create("./CA_results")
writeRaster(1-WDLens.pred, filename = "./CA_results/WDLens.tif")
...
#+ Data setup --------------------------------------------------------------
# Extract gridded variables at GeoSurvey locations
geogrid=extract(t,geos1.proj)#extract 28 k
...
plot(varImp(HSP1.rf,scale=F), main = "Variable Importance Random Forest Settlements", top=30, cex=1.2, col= "black")
...
#k nearest neighbor
HSP1.kn <- train(HSP1 ~., data=hsp1Train, 
                 method = "knn", 
                 metric= "ROC",
                 #preProc = c("center", "scale"), 
                 trControl = objControl)
confusionMatrix((HSP1.kn))
HSP1.imp <- varImp(HSP1.kn, useModel = FALSE)
plot(HSP1.imp, top=30, cex= 1.2, col = "black", main = "Variable Importance k nearest neighbor Settlements")
HSP1kn.pred <- predict(t, HSP1.kn, type="prob")

#ensemble regression glmnet (elastic net)
pred <- stack(1-HSP1rf.pred, 
              1-HSP1gbm.pred, 1-HSP1nn.pred, 1-HSP1kn.pred)
names(pred) <- c("HSP1rf","HSP1gbm", "HSP1nn", "HSP1kn")
geospred <- extract(pred, geos1.proj)

# presence/absence of settlements (present = Y, absent = N)
HSP1ens <- cbind.data.frame(geos1$HSP, geospred)
...
HSP1.eval
plot(HSP1.eval, 'ROC')
predictions= predict(HSP1.ens, HSP1ensTest)
confusionMatrix(predictions, HSP1ensTest [,1])
#for all data RF--------------------------------
objControl <- trainControl(method='cv', number=10, returnResamp='none', 
                           allowParallel = TRUE, classProbs = TRUE,
                           summaryFunction= twoClassSummary)
...
#k nearest neighbor
HSP.kn <- train(HSP1 ~., data=hsp1dat, 
                method = "knn", 
                metric= "ROC",
                #preProc = c("center", "scale"), 
                trControl = objControl)
confusionMatrix(HSP.kn)

HSPkn.pred <- predict(t, HSP.kn, type="prob")

#ensemble regression glmnet (elastic net)
pred <- stack(1-HSPrf.pred, 
              1-HSPgbm.pred, 1-HSPnn.pred, 1-HSPkn.pred)
names(pred) <- c("HSPrf","HSPgbm", "HSPnn", "HSPkn")
geospred <- extract(pred, geos1.proj)
...
dir.create("CA_results", showWarnings=F)
writeRaster(1-HSPens.pred, filename = "./CA_results/hsppred.tif", overwrite=TRUE )

hsppred=1-HSPens.pred
hsppred=scale(hsppred, center=TRUE,scale=TRUE)
t=addLayer(t,hsppred)
names(t)[62]= "HSPpred"

#load cropland geosurvey
geos1= download("https://www.dropbox.com/s/5s1norwr4rlubog/central-african-highlands.zip?raw=1","./CA_data/central-african-highl_hsp.zip", mode="wb")
geos1= unzip("./central-african-highlands.zip", exdir= "./CA_data", overwrite=T)
geos1 <- read.table(paste(dat_dir, "/central-african-highlands.csv", sep=""), header=T, sep=",") 
#geos1 <- na.omit(geos1)
# Project GeoSurvey coords to grid CRS for 28000+
geos1.proj <- as.data.frame(project(cbind(geos1$Longitude, geos1$Latitude), "+proj=laea +ellps=WGS84 +lon_0=20 +lat_0=5 +units=m +no_defs"))
colnames(geos1.proj) <- c("x","y")
...
#k nearest neighbour
CRP1.kn <- train(CRP1 ~., data=crp1Train, 
                 method = "knn", 
                 metric= "ROC",
                 #preProc = c("center", "scale"), 
                 trControl = objControl)
confusionMatrix(CRP1.kn)

CRP1kn.pred <- predict(t,CRP1.kn, type="prob")

#for cropland
#ensemble regression glmnet (elastic net)
pred <- stack(1-CRP1rf.pred, 
              1-CRP1gbm.pred, 1-CRP1nn.pred, 1-CRP1kn.pred)
names(pred) <- c("CRP1rf","CRP1gbm", "CRP1nn", "CRPkn")
geospred <- extract(pred, geos1.proj)

# presence/absence of Cropland (present = Y, absent = N)
CRP1ens <- cbind.data.frame(geos1$CRP, geospred)
...
plot(CRP1.eval, 'ROC')
#Confusion matrix
predictions <- predict(CRP1.ens, CRP1ensTest)
confusionMatrix(predictions, CRP1ensTest [,1])
...
CRPnn.pred <-predict(t, CRP.nn, type = "prob") ## spatial predictions
#K nearest neighbor
CRP.kn <- train(CRP1 ~., data=crp1dat, 
                method = "knn", 
                metric= "ROC",
                #preProc = c("center", "scale"), 
                trControl = objControl)
confusionMatrix(CRP.kn)
CRPkn.pred <-predict(t, CRP.kn, type = "prob") ## spatial predictions

#ensemble regression glmnet (elastic net)
pred <- stack(1-CRPrf.pred, 
              1-CRPgbm.pred, 1-CRPnn.pred, 1-CRPkn.pred)
names(pred) <- c("CRPrf","CRPgbm", "CRPnn", "CRPkn")
geospred <- extract(pred, geos1.proj)
...
writeRaster(CRPens.pred, filename="./CA_results/CRPens.tif")