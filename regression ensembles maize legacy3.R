...
maizeY0_laea=spTransform(maizeY0, CRS("+proj=laea +lat_0=5 +lon_0=20 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0")) # reproject vector file to LAEA
plot(maizeY0_laea, axes=TRUE)


#data frame for all maize sites n=325
m2_2015=as.data.frame(maizeY0_laea)
m2=m2_2015[c("x","y", "Grain_Yiel")]
m2xy=m2[,1:2] #coords only

#+ Data setup --------------------------------------------------------------
#for 325 samples baseline yield (control yields)
...
step <- trainControl(method = "cv", number = 3)
