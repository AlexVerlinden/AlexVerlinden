...
dir.create("NG_sys", showWarnings=F)
dat_dir="./NG_sys"

# Download
download.file("https://www.dropbox.com/s/uej0ur6nqr8shg9/Crop_scout_2016_crop.csv?dl=0", "./NG_sys/Crop_scout_2016_crop.csv", mode="wb")
# note that 0 and 1 are not ok for Caret for classifications, should be N and Y or similar
ngcrop <- read.csv(paste(dat_dir, "/Crop_scout_2016_crop.csv", sep= ""), header=T, sep=",")

maize <- as.data.frame(table(ngcrop$cereal.maize, ngcrop$legume_pa, ngcrop$root_pa, ngcrop$otherc_pa, ngcrop$livestock_pa))
...
sorghum= as.data.frame(table(ngcrop$cereal.sorgum, ngcrop$legume_pa, ngcrop$root_pa, ngcrop$otherc_pa, ngcrop$livestock_pa))
colnames(sorghum) <- c("Sorghum","Legume","Root","Other","Livestock","Freq")

alluvial(sorghum[,1:5], freq=sorghum$Freq, border=NA,
         hide = sorghum$Freq < quantile(sorghum$Freq, 0.50),
         col=ifelse(sorghum$Sorghum == "Y", "red", "gray"))