#' Ensemble predictions of Ethiopia GeoSurvey cropland, woody vegetation cover,
...
#download Geosurvey data  23000 +
download.file("https://www.dropbox.com/s/ysh55mdy5ux9698/ET_GS_240217.zip?raw=1", "./ET_data/ET_GS_240217.zip", mode="wb")
unzip("./ET_data/ET_GS_240217.zip", exdir= "./ET_data", overwrite=T)
geos1 <- read.table(paste(dat_dir, "/ET_GS_240217.csv", sep=""), header=T, sep=",")
geos1 <- na.omit(geos1)
# Project GeoSurvey coords to grid CRS for 23000+
...
# download ET Gtifs (!!!~2.200 GB) and stack in raster
download.file("https://www.dropbox.com/s/bsiugyzgda8lgv1/ET_GRIDS.zip?raw=1", "./ET_data/ET_GRIDS.zip", mode="wb")
...
geogrid=extract(t, geos1.proj) #extract 23k
...
#create results
dir.create("./ET_results")
writeRaster(1-WDLens.pred, filename = "./ET_results/WDLens.tif")
WDLens=1-WDLens.pred
WDLens=scale(WDLens, center = TRUE, scale =TRUE)
t=addLayer(t,WDLens)
names(t)[39]="WDLENS" #check numbers
...
geogrid=extract(t,geos1.proj)#extract 23 k
# Assemble dataframes
...
# presence/absence of settlements (present = Y, absent = N)
HSPens <- cbind.data.frame(geos1$BLD, geospred)
...
writeRaster(1-HSPens.pred, filename = "./ET_results/HSPens.tif", overwrite=TRUE)
HSPens=1-HSPens.pred
HSPens=scale(HSPens, center = TRUE, scale =TRUE)
t=addLayer(t, HSPens)
names(t)[40]= "HSPENS"

#predict cropland on 23k
...
writeRaster(1-CRPens.pred, filename = "./ET_results/cropens.tif")