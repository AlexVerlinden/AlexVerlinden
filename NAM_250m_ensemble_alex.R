...
bush <- na.omit(bush)
# download Namibia 250 m Gtifs (~ 310 Mb) and stack in raster
download.file("https://www.dropbox.com/s/8mpat9mohyst1fj/NAMGRIDS_250.zip?dl=0", "./NAM_bush/NAMGRIDS_250.zip", mode="wb")
unzip("./NAM_bush/NAMGRIDS_250.zip", exdir=dat_dir, overwrite=T)
...
bush60.glm=train(bush60 ~ ., data=bush6Train, family= "binomial",method="glmnet",metric="Accuracy", trControl=objControl)

predictions <- predict(bush60.glm, bush6Test, type="prob")
...
rf=writeRaster(bushens0,filename="./NAM_Results/bush_0%_pred.tif", format= "GTiff", overwrite = TRUE)
