...
#glmnet
#glmnet has advantage that you can see the positive and negative influence of covariates
...
bushrf.pred <- predict(grid, bush6.rf, type = "prob") ## spatial predictions
plot(1-bushrf.pred)
...
bush6gbm.pred=predict(grid, bush6.gbm, type = "prob")
...
bush60nn.pred <- predict(grid, bush60.nn, type = "prob") ## spatial predictions
...
bush.thld1 <- threshold(bush.eval, 'kappa')
bushens.pred <- predict(bush60.preds, bush60.ens, type="prob") ## spatial prediction
bushmask <- 1-bushens.pred > bush.thld
bushmask1 <- 1-bushens.pred > bush.thld1
plot(bushmask, axes = F, legend = F)
plot(bushmask1, axes = F, legend = F)
...
#write tiff
rf=writeRaster(1-bushens.pred,filename="./NAM_Results/bush_60%_raw.tif", format= "GTiff", overwrite = TRUE)


rf=writeRaster(bushmask,filename="./NAM_Results/bush_60%_kappa.tif", format= "GTiff", overwrite = TRUE)
...
bush3.rf <- train(bush30 ~ ., data = bush3Train,
                  method = "rf",
                  trControl = oob)
...
bushrf3.pred <- predict(grid, bush3.rf, type = "prob") ## spatial predictions
...
bush3.gbm <- train(bush30 ~ ., data = bush3Train,
                   method = "gbm",
                   trControl = gbm)
...
bush3gbm.pred=predict(grid, bush3.gbm, type = "prob")
...
#+ Plot predictions by GeoSurvey variables ---------------------------------
# bush encroachment 30-60 % prediction plots
...
plot(impens30, main = "contribution to Regression Ensemble (expert) 30-60%")
...
#write tiff
rf=writeRaster(bushmask,filename="./NAM_results/bush_30%.tif", format= "GTiff", overwrite = TRUE)
rf=writeRaster(bushmask1,filename="./NAM_results/bush_30%_kappa.tif", format= "GTiff", overwrite = TRUE)
...
bush15rf.pred <- predict(grid, bush15.rf, type = "prob") ## spatial predictions
...
bush15gbm.pred=predict(grid, bush15.gbm, type = "prob")
...
bush15.nn <- train(bush15 ~ ., data = bush15Train,
                   method = "nnet",
                   trControl = nn)
...
bush15nn.pred <- predict(grid, bush15.nn, type = "prob") ## spatial predictions
...
impens=varImp(bush15.ens)
plot(impens, main = "Regression contribution to Ensemble (expert), 10-30%")
...
rf=writeRaster(bushmask1,filename="./NAM_Results/bush_15%_kappa.tif", format= "GTiff", overwrite = TRUE)
rf=writeRaster(bushens.pred,filename="./NAM_Results/bush_15%_pred.tif", format= "GTiff", overwrite = TRUE)
###___________________________________________________
#woody cover 0-10%
#+ Random forests <randomForest> -------------------------------------------
# out-of-bag predictions
...
bush1rf.pred <- predict(grid, bush1.rf, type = "prob") ## spatial predictions
...
bush1gbm.pred=predict(grid, bush1.gbm, type = "prob")
...
bush10nn.pred <- predict(grid, bush10.nn, type = "prob") ## spatial predictions
#variable contribution
...
rf=writeRaster(bushmask1,filename="./NAM_Results/bush_10%_kappa.tif", format= "GTiff", overwrite = TRUE)
...
bush0rf.pred <- predict(grid, bush0.rf, type = "prob") ## spatial predictions
...
bush0gbm.pred=predict(grid, bush0.gbm, type = "prob")
...
bush0nn.pred <- predict(grid, bush0.nn, type = "prob") ## spatial predictions
...
rf=writeRaster(bushmask1,filename="./NAM_Results/bush_0%_kappa.tif", format= "GTiff", overwrite = TRUE)
rf=writeRaster(bushens.pred,filename="./NAM_Results/bush_0%_pred.tif", format= "GTiff", overwrite = TRUE)
