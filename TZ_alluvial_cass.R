# Alluvial diagram of TZ cassava cropping systems
# Alex Verlinden after M. Walsh, February 2016
...
dir.create("TZ_sys", showWarnings=F)
dat_dir="./TZ_sys"

# Download
download.file("https://www.dropbox.com/s/02g8dmzvr18nyx3/Crop_TZ_JAN_2017.csv.zip?dl=0", "./TZ_sys/Crop_TZ_JAN_2017.csv.zip", mode="wb")
unzip("./TZ_sys/Crop_TZ_JAN_2017.csv.zip", exdir=dat_dir, overwrite=T)

tzcrop <- read.csv(paste(dat_dir, "/Crop_TZ_JAN_2017.csv", sep= ""), header=T, sep=",")
#maize
tzmaize=as.data.frame(table(tzcrop$maize,tzcrop$legume_pa, 
                            tzcrop$root_crops_pres,
                            tzcrop$other_crops,tzcrop$livestock_sign))
colnames(tzmaize)=c("Maize", "Legumes", "Rootcrops", "Other crops", "Livestock", "Freq")

# <alluvial> diagram ------------------------------------------------------
alluvial(tzmaize[,1:5], freq=tzmaize$Freq, border=NA,
         hide = tzmaize$Freq < quantile(tzmaize$Freq, 0.50),
         col=ifelse(tzmaize$Maize == "Y", "red", "gray"))

#Sunflower
tzsunflower=as.data.frame(table(tzcrop$Sunflower,tzcrop$legume_pa, 
                            tzcrop$root_crops_pres, tzcrop$maize,
                            tzcrop$other_crops,tzcrop$livestock_sign))
colnames(tzsunflower)=c("Sunflower", "Legumes", "Rootcrops", "Maize","Other crops", "Livestock", "Freq")

# <alluvial> diagram ------------------------------------------------------
alluvial(tzsunflower[,1:6], freq=tzsunflower$Freq, border=NA,
         hide = tzsunflower$Freq < quantile(tzsunflower$Freq, 0.50),
         col=ifelse(tzsunflower$Sunflower == "Y", "red", "gray"))
# sorghum
tzsorghum=as.data.frame(table(tzcrop$sorghum,tzcrop$legume_pa, 
                                tzcrop$root_crops_pres, tzcrop$maize,
                                tzcrop$other_crops,tzcrop$livestock_sign))
colnames(tzsorghum)=c("Sorghum", "Legumes", "Rootcrops", "Maize","Other crops", "Livestock", "Freq")

# <alluvial> diagram ------------------------------------------------------
alluvial(tzsorghum[,1:6], freq=tzsorghum$Freq, border=NA,
         hide = tzsorghum$Freq < quantile(tzsorghum$Freq, 0.50),
         col=ifelse(tzsorghum$Sorghum == "Y", "red", "gray"))

#cassava
tzcassava <- as.data.frame(table(tzcrop$root.cassava, tzcrop$maize,
                             tzcrop$legume_pa, tzcrop$root.other, 
                             tzcrop$other_crops, tzcrop$livestock_sign))
colnames(tzcassava) <- c("Cassava","maize", "Legume","Root","Other","Livestock","Freq")
tzcassava=subset(tzcassava, Cassava="Y")
# <alluvial> diagram ------------------------------------------------------
alluvial(tzcassava[,1:6], freq=tzcassava$Freq, border=NA,
         hide = tzcassava$Freq < quantile(tzcassava$Freq, 0.50),
         col=ifelse(tzcassava$Cassava == "Y", "red", "gray"))

#Banana
tzbanana= as.data.frame(table(tzcrop$Banana, tzcrop$cereal_pres, tzcrop$legume_pa, tzcrop$root_crops_pres, tzcrop$root.cassava, tzcrop$livestock_sign))
colnames(tzbanana) <- c("Banana","Cereals","Legumes","Rootcrops", "Cassava","Livestock","Freq")

alluvial(tzbanana[,1:6], freq=tzbanana$Freq, border=NA,
         hide = tzbanana$Freq < quantile(tzbanana$Freq, 0.50),
         col=ifelse(tzbanana$Banana == "Y", "red", "gray"))
