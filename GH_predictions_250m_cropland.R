...
dat_dir <- "./GH_data"
#note for download depending on versions of R  by download dl=0 might have to be set to raw=1
...
geogrid=extract(t, geos1.proj) #extract 13k
...
WDL1ens <- cbind.data.frame(geos1$WLD, geospred)
...
WDLens <- cbind.data.frame(geos1$WLD, geospred)
...
writeRaster(1-WDLens.pred, filename = "./GH_results/WDLens.tif")
GH_WDL=raster(1-WDLens.pred)
WDLens=scale(GH_WDL, center = TRUE, scale =TRUE)
t=addLayer(t,WDLens)
names(t)[39]="WDLENS"
...
geogrid=extract(t,geos1.proj)#extract 113 k
# Assemble dataframes
...
# presence/absence of settlements (present = Y, absent = N)
HSPens <- cbind.data.frame(HSP1, geospred)
...
HSPp <- subset(HSP.test, HSP1=="Y", select=c(Y))
HSPa <- subset(HSP.test, HSP1=="N", select=c(Y))
HSP.eval <- evaluate(p=HSPp[,1], a=HSPa[,1]) ## calculate ROC's on test set <dismo>
HSP.eval

#spatial predictions
HSPens.pred <- predict(pred, HSP.ens, type="prob") 
writeRaster(1-HSPens.pred, filename = "./GH_results/HSPens.tif")
HSPens=1-HSPens.pred
t=addLayer(t, HSPens)
names(t)[40]= "HSPENS"

#predict cropland on 13k
...
writeRaster(1-CRPens.pred, filename = "./GH_results/cropens.tif")
